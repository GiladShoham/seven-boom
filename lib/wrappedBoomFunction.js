...
function getWrappedUnauthorized(func, guidFunc, timeThrownFunc){
  return function(message, scheme, attributes, errorCode, publicData) {
    let mergedData = {};
    mergedData.public = publicData;
    let result =  func(message, scheme, attributes);
    result.output.payload.timeThrown = timeThrownFunc();
    result.output.payload.guid = guidFunc();
    result.output.payload.errorCode = errorCode;
    return result;
